...
&sql.insert.unspend.log [v(d.xpas)]=INSERT INTO xp_log (target_objid, target_name, enactor_objid, enactor_name, xp_type, xp_amt, trait_category, trait_name, trait_value, trait_value_old, action, reason) VALUES ('[u(.objid, %0)]', '[u(f.sql.escape, name(%0))]', '[u(.objid, %1)]', '[u(f.sql.escape, name(%1))]', '[u(f.sql.escape, %2)]', %3, '[u(f.sql.escape, lcstr(first(%4, .)))]', '[u(f.sql.escape, lcstr(rest(%4, .)))]', '[u(f.sql.escape, %6)]', '[u(f.sql.escape, %7)]', 'unspend', '[u(f.sql.escape, %5)]')
