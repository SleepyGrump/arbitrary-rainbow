...
Escaped some other stuff, and removed an escape from the "freebie" command because it was already being escaped in the SQL command. Same with the spend command, the award command, and the convert command - some spots were escaped, some weren't, so centralizing them in the insert works better.
...
&f.xp/spend [v(d.xpas)]=strcat(setq(p, first(%0, =)), setq(v, rest(%0, =)), setq(n, rest(%qp, /)), setq(p, first(%qp, /)), if(not(t(%qn)), strcat(setq(n, %qp), setq(p, %#)), setq(p, pmatch(%qp))), setq(w, ulocal(v(d.sfp)/f.find-sheet, %qp)), setq(s, ulocal(v(d.sfp)/f.statpath.workhorse, %qn, %qp)), setq(t, ulocal(.value, %qw, %qs)), if(not(t(%qv)), setq(v, ulocal(f.stat.next_rank, %qw, _%qs))), setq(c, ulocal(f.cost.calculator, %qw, %qt, %qv, %qs)), setq(x, get(%qw/_special.beats.normal)), case(1, cand(strmatch(u(pref.player_spend), off), not(isstaff(%#))), u(.msg, xp/spend, Player spending is off. Ask staff to spend your stats.), t(setr(e, ulocal(validate.player, %qp, %qw, 1))), u(.msg, xp/spend, %qe), t(setr(e, ulocal(validate.stat, %qw, %qs, %qt, %qv))), u(.msg, xp/spend, %qe), t(setr(e, ulocal(validate.value, %qs, %qt, %qv))), u(.msg, xp/spend, %qe), t(setr(e, ulocal(validate.cost, %qc))), u(.msg, xp/spend, %qe), t(setr(e, ulocal(validate.spend, %qc, %qx))), u(.msg, xp/spend, %qe), t(setr(e, ulocal(validate.restricted, %qs, %qv))), u(.msg, xp/spend, %qe), strmatch(setr(e, sql(u(sql.insert.spend, %qp, %#, normal, mul(%qc, v(d.beat-to-exp)), %qs, null(null), %qv, %qt))), #-*), u(.msg, xp/spend, SQL connection is down%; spend not processed), strmatch(setr(e, ulocal(v(d.sfp)/f.setstat.workhorse, %qw, %qs, %qv)), #-*), u(.msg, xp/spend, Error from Setstat: %qe), strcat(setq(e, set(%qw, _special.beats.normal:[first(%qx)] [sub(rest(%qx), mul(%qc, v(d.beat-to-exp)))])), if(strmatch(%qe, #-*), u(.msg, xp/spend, Error while setting new xp: %qe %r%(Original XP attribute: %qx%))), setq(n, statname(%qp/%qn)), u(.msg, xp/spend, strcat(if(strmatch(%qp, %#), Your, [name(%qp)]'s), %b, %qn, %b, has been set, case(u(v(d.sfp)/f.get-class, %qs), list,, %bto '%qv'), %b, for %qc Experience.)),)))
