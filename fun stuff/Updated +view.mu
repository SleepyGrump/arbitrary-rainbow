...
&layout.view #6=if(hasattrp(%0, %1), if(strmatch(setr(2, xget(%0, %1)), *wheader*), u(%0/%1), strcat(wheader(titlestr(edit(ucstr(%1), VIEW-,, ~, %b, _, %b))), if(strmatch(%q2, \%r*), %r, %r%r), wrap(strcat(u(%0/%1)), sub(width(%3), 6), Left, space(3), space(3)), %r%r, wfooter())), There is no such view on that item (%0/%1).)
