...
Messaging is vital to roleplay, whether through text, phone, comm, or telepathy. We've done our best to keep it as simple, yet flexible, as possible.
	...
	msg/<type> <player>=<text> - send a <type> kind of message (examples: text, HUD, comm, telepathic, etc)
	msg <text> - send message with the same settings as the last one

After the first msg is sent, it remembers the last person (or group of people!) you messaged, and the last type of message you sent.
...
Some people like to customize messages to stand out better. To customize your sent messages:
	...
	Now whenever you type msg/ra name=message, they'll see a bright red RED ALERT message. The letters "ra" effectively become a shortcut for "red alert", and your friends see the message as urgent as you meant to show it.
...
@@ There is probably a bug here - unknown switches don't fall through correctly.

&switch.msg MSG=@break t(xget(%1, msg-receive-off))={@pemit %1=alert(MSG) You cannot send messages while your ability to receive messages is turned off. Type msg/on to enable the message system.;};@eval setq(T, if(not(strmatch(%0, %b*)), ucstr(strip(first(%0), /)), xget(%1, last-msg-type)));@eval setq(L, switch(trim(%0), /* *=*, first(rest(%0), =), *=*, first(%0, =), edit(xget(%1, last-msg-target), |, %b)));@eval setq(V, trim(switch(%0, *=*, rest(%0, =), %0)));@eval setq(P, iter(%qL, case(itext(0), me, %1, pmatch(itext(0))),, |));@eval iter(%qP, if(not(t(itext(0))), setq(E, %qE Could not find player '[if(t(%qL), extract(%qL, inum(0), 1), itext(0))]' ([itext(0)]).)), |);@break t(squish(trim(%qE)))={@pemit %1=alert(MSG) %qE;};@set %1=last-msg-target:%qP;@if t(%qT)={@set %1=last-msg-type:%qT;};@pemit %1=strcat(alert(To [itemize(iter(%qP, name(itext(0)), |, |), |)]), %b, if(and(t(%qT), hasattr(%1, msg-send-%qT)), s(xget(%1, msg-send-%qT))%b, if(t(%qT), %[%qT%]%b)), switch(%qV, :*, name(%1) [rest(%qV, :)], ;*, name(%1)[rest(%qV, ;)], |*, rest(%qV, |), name(%1) sends%, "%qV"));@dolist/delimit | [setdiff(setunion(%qP, %qP, |), %1, |)]={@switch/first 1=hasattr(##, msg-hide-%1), {}, hasattr(##, msg-block-%1), {@pemit %1=alert(MSG NOT SENT) [name(##)] has blocked you[if(match(setr(B, u(##/msg-block-%1)), 1), ., %bbecause: %qB)];}, and(or(t(xget(##, msg-receive-off)), not(hasflag(##, connect))), gte(words(lattr(##/_msg-*)), 50)), {@pemit %1=alert(MSG NOT SENT) [name(##)] is currently inaccessible to the message system. Your message could not be stored because the message system can only hold the last 50 messages and [poss(##)] message queue is full. You will need to re-send your message later.;}, or(t(xget(##, msg-receive-off)), not(hasflag(##, connect))), {&_msg-[secs()] ##=strcat(alert(if(match(%1, ##), To [itemize(iter(%qP, name(itext(0)), |, |), |)], From [name(%1)])), %b, if(and(t(%qT), hasattr(%1, msg-send-%qT)), s(xget(%1, msg-send-%qT))%b, if(t(%qT), %[%qT%]%b)), switch(%qV, :*, name(%1) [rest(%qV, :)], ;*, name(%1)[rest(%qV, ;)], |*, rest(%qV, |), name(%1) sends%, "%qV"));@pemit %1=alert(MSG NOT RECEIVED) [name(##)] is currently inaccessible to the message system. Queuing message for delivery once [setr(S, subj(##))] [case(%qS, they, return, returns)].;}, {@pemit ##=strcat(alert(From [name(%1)][if(gt(words(%qP, |), 1), %bto [itemize(iter(%qP, name(itext(0)), |, |), |)])]), %b, if(and(t(%qT), hasattr(%1, msg-send-%qT)), s(xget(%1, msg-send-%qT))%b, if(t(%qT), %[%qT%]%b)), switch(%qV, :*, name(%1) [rest(%qV, :)], ;*, name(%1)[rest(%qV, ;)], |*, rest(%qV, |), name(%1) sends%, "%qV"));}}
