...
&layout.advantages [v(d.animals)]=strcat(%r, wdivider(Advantages), %r, ulocal(layout.row, Willpower, ulocal(f.get-trait, resolve, composure), Health, ulocal(f.get-trait, size, stamina), Initiative, ulocal(f.get-trait, dexterity, composure)), %r, ulocal(layout.row, Defense, ulocal(f.get-defense), Speed, ulocal(f.get-trait, strength, dexterity, ulocal(f.get-species-factor)), Size, ulocal(f.get-size)))
